...
import { normalizePath, validatePathWithinBase, ACM_ROOT } from "../paths.js";
...
describe("validatePathWithinBase", () => {
  it("accepts paths within base", async () => {
    const result = await validatePathWithinBase(
      path.join(ACM_ROOT, "kb/README.md"),
      ACM_ROOT
    );
  ...
  it("rejects absolute paths outside base", async () => {
    const result = await validatePathWithinBase("/etc/passwd", ACM_ROOT);
    expect(result.valid).toBe(false);
  });

  it("handles non-existent paths gracefully", async () => {
    const result = await validatePathWithinBase(
      path.join(ACM_ROOT, "nonexistent-file.md"),
      ACM_ROOT
    );
    expect(result.valid).toBe(true);
  });
